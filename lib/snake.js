(function () {
  ...
  var Snake = SnakeGame.Snake = function (game, color, startHoriz) {
    this.game = game;
    this.segmentRadius = 5;
    this.color = color;
    this.spd = 2;
    this.foodsEaten = 0;
    this.startHoriz = startHoriz;

    this.reset();
  }

  Snake.prototype.step = function (otherSnake) {
    this.move();
    this.checkFoods();
    this.checkCollisions(otherSnake);
  }
  ...
  Snake.prototype.checkFoods = function () {
    this.game.foods.forEach (function (food) {
      if (SnakeGame.Util.dist(this.pos, food.pos) < SnakeGame.Food.rad + this.segmentRadius) {
        this.foodsEaten++;
        this.game.eatFood(this);
        food.getEaten(this);
      }
    }.bind(this));
  }

  Snake.prototype.checkCollisions = function (otherSnake) {
    ...
    if (otherSnake) {
      otherSnake.segments.forEach (function (segment) {
        if (SnakeGame.Util.dist(this.pos, segment) < this.segmentRadius) {
          this.reset();
        }
      }.bind(this))
  ...
  Snake.prototype.reset = function () {
    this.pos = [this.startHoriz, this.game.dim / 2]
    this.segments = [];
    this.rotation = 180;
    this.vel = [0, -1];
    this.length = 50;
